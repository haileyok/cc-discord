...
The loopback Polytoken HTTP daemon requires a per-session bearer token. `polytoken sessions --format json` supplies each daemon's credential-file path; the client accepts only a regular 0600 JSON file with `version: 1`, `kind: "bearer"`, and a non-empty token, then sends `Authorization: Bearer …` on every HTTP request and SSE connection. Keep the health server on `127.0.0.1`; do not expose it to a network. A compromised local account, project checkout, Slack workspace member, or prompt-injected file is outside the bridge's defense boundary. Agent attachment markers are not a sandbox expansion: shell access already exists.
