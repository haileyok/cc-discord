...
- **`AllowedUntrustedInternalConnections` required for slash command webhooks.** Mattermost blocks outgoing HTTP requests to private IPs by default. The admin must add the bridge's bind address to `ServiceSettings.AllowedUntrustedInternalConnections` in `config.json` (or via System Console > Environment > Developer). When Mattermost runs in Docker, add `172.17.0.1` (docker bridge IP); when native, add `127.0.0.1 localhost`. Without this, all slash commands fail with "command with a trigger of '/X' failed".
- **Docker networking for slash commands.** When Mattermost is in Docker, `localhost` from the container is the container itself — not the host. The bridge must bind to the docker bridge IP (`172.17.0.1`) via `--host 172.17.0.1`, and slash commands must be registered with `--url http://172.17.0.1:8787`. `scripts/start-mattermost.sh` defaults to `172.17.0.1`; override with `BRIDGE_HOST` env var. This IP is only routable from the docker bridge network, not external interfaces.
